...
const rangeSchema = z.enum(["1D", "7D", "30D", "90D", "1Y", "MAX"]).catch("7D");
const idSchema = z
  .string()
  .min(1)
  .max(64)
    ...
    handlers: {
      GET: async ({ params, request }) => {
        ...
        try {
          ...
          return Response.json(coin, { headers: { "cache-control": "public, max-age=15" } });
        } catch {
          return Response.json({ error: "Coin data unavailable." }, { status: 503 });
        }
      },
    },
  },
